/* eslint-disable */
import React from "react";

const NavList = ({ route, setRoute }) => {
  return (
    <>
      <div class="menu-container flex-grow-1">
        <ul id="menu" class="menu">
          <li>
            <a href="/dashboard">
              <i data-acorn-icon="shop" class="icon" data-acorn-size="18"></i>
              <span class="label">Dashboard</span>
            </a>
          </li>
          <li>
            <a href="#products" data-href="Products.html">
              <i
                data-acorn-icon="cupcake"
                class="icon"
                data-acorn-size="18"
              ></i>
              <span class="label">Products</span>
            </a>
            <ul id="products">
              <li
                onClick={() => {
                  setRoute("/dashboard?screen=products");
                }}
              >
                <a href="/dashboard?screen=products">
                  <span class="label">List</span>
                </a>
              </li>
              <li
                onClick={() => {
                  setRoute("/dashboard?screen=product-details");
                }}
              >
                <a href="/dashboard?screen=product-details">
                  <span class="label">Detail</span>
                </a>
              </li>
            </ul>
          </li>
          <li>
            <a href="#orders" data-href="Orders.html">
              <i data-acorn-icon="cart" class="icon" data-acorn-size="18"></i>
              <span class="label">Orders</span>
            </a>
            <ul id="orders">
              <li
                onClick={() => {
                  setRoute("/dashboard?screen=orders");
                }}
              >
                <a href="/dashboard?screen=orders">
                  <span class="label">List</span>
                </a>
              </li>
              <li
                onClick={() => {
                  setRoute("/dashboard?screen=order-details");
                }}
              >
                <a href="/dashboard?screen=order-details">
                  <span class="label">Detail</span>
                </a>
              </li>
            </ul>
          </li>
          {/* <li>
            <a href="#customers" data-href="Customers.html">
              <i data-acorn-icon="user" class="icon" data-acorn-size="18"></i>
              <span class="label">Customers</span>
            </a>
            <ul id="customers">
              <li>
                <a href="Customers.List.html">
                  <span class="label">List</span>
                </a>
              </li>
              <li>
                <a href="Customers.Detail.html">
                  <span class="label">Detail</span>
                </a>
              </li>
            </ul>
          </li> */}
          {/* <li>
            <a href="#storefront" data-href="Storefront.html">
              <i data-acorn-icon="screen" class="icon" data-acorn-size="18"></i>
              <span class="label">Storefront</span>
            </a>
            <ul id="storefront">
              <li>
                <a href="Storefront.Home.html">
                  <span class="label">Home</span>
                </a>
              </li>
              <li>
                <a href="Storefront.Filters.html">
                  <span class="label">Filters</span>
                </a>
              </li>
              <li>
                <a href="Storefront.Categories.html">
                  <span class="label">Categories</span>
                </a>
              </li>
              <li>
                <a href="Storefront.Detail.html">
                  <span class="label">Detail</span>
                </a>
              </li>
              <li>
                <a href="Storefront.Cart.html">
                  <span class="label">Cart</span>
                </a>
              </li>
              <li>
                <a href="Storefront.Checkout.html">
                  <span class="label">Checkout</span>
                </a>
              </li>
              <li>
                <a href="Storefront.Invoice.html">
                  <span class="label">Invoice</span>
                </a>
              </li>
            </ul>
          </li> */}
          {/* <li>
            <a href="Shipping.html">
              <i
                data-acorn-icon="shipping"
                class="icon"
                data-acorn-size="18"
              ></i>
              <span class="label">Shipping</span>
            </a>
          </li>
          <li>
            <a href="Discount.html">
              <i data-acorn-icon="tag" class="icon" data-acorn-size="18"></i>
              <span class="label">Discount</span>
            </a>
          </li> */}
          <li
            onClick={() => {
              setRoute("/dashboard?screen=settings");
            }}
          >
            <a href="/dashboard?screen=settings">
              <i data-acorn-icon="gear" class="icon" data-acorn-size="18"></i>
              <span class="label">Settings</span>
            </a>
          </li>
        </ul>
      </div>
      {/* <!-- Menu End -->

          <!-- Mobile Buttons Start --> */}
      <div class="mobile-buttons-container">
        {/* <!-- Menu Button Start --> */}
        <a href="#" id="mobileMenuButton" class="menu-button">
          <i data-acorn-icon="menu"></i>
        </a>
        {/* <!-- Menu Button End --> */}
      </div>
      {/* <!-- Mobile Buttons End --> */}
    </>
  );
};

export default NavList;
